...
/**
 * The watch-side config activity for {@link TestService}, which allows for setting
...
public class ComplicationConfigActivity extends Activity implements View.OnClickListener {
    ...
    /**
     * Used by associated watch face ({@link TestService}) to let this
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mLeftComplicationId =
                TestService.getComplicationId(ComplicationLocation.LEFT);
        mRightComplicationId =
                TestService.getComplicationId(ComplicationLocation.RIGHT);

        mBottomComplicationId = TestService.getComplicationId(ComplicationLocation.BOTTOM);

        mWatchFaceComponentName =
                new ComponentName(getApplicationContext(), TestService.class);
    ...
    public void retrieveInitialComplicationsData() {
        ...
        final int[] complicationIds = TestService.getComplicationIds();
    ...
    private void launchComplicationHelperActivity(ComplicationLocation complicationLocation) {
        ...
        mSelectedComplicationId =
                TestService.getComplicationId(complicationLocation);

        if (mSelectedComplicationId >= 0) {

            int[] supportedTypes =
                    TestService.getSupportedComplicationTypes(
